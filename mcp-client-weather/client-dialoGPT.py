...
class MCPClient:
    def __init__(self):
        ...
        # ← LOCAL DialoGPT (NO API KEY!)
        print("🦙 Loading DialoGPT locally...")
        model_id = "microsoft/DialoGPT-medium"  # 8B = fast on most machines
    ...
    def _call_dialoGPT(self, messages, tools=None):
        """Call local DialoGPT with tool support"""
        ...
        # Simple tool detection (DialoGPT will output JSON)
    ...
    async def process_query(self, query: str) -> str:
        ...
        # First DialoGPT call
        llama_response = self._call_dialoGPT(messages, tools)
        ...
        if llama_response["tool_calls"]:
            for tool_call in llama_response["tool_calls"]:
                ...
                # Second DialoGPT call
                llama_response = self._call_dialoGPT(messages)
                final_text.append(llama_response["content"])
        
        return "\n".join(final_text)

    async def chat_loop(self):
        print("\n🦙 LOCAL DialoGPT MCP Client Started! (No API needed)")
